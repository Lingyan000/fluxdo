    ...
    if ((statusCode == 403 || statusCode == 429) &&
      ...
      if (result == true) {
        ...
        try {
          ...
          // CF 盾常伴随会话 cookie 轮换,内存里的 CSRF 可能已与新会话不匹配
          // (旧 CSRF + 新 cookie = mutation 继续 403,重启才恢复)。清掉后
          // retry 走完整拦截器链,RequestHeaderInterceptor 会用新 clearance
          // 自动刷新 /session/csrf。
          CsrfTokenService().clearCsrfToken();
          retryOptions.headers.remove('X-CSRF-Token');
          ...
          // 模拟重启:CF 盾说明当前会话身份包(fingerprint 上报、_rt/
          // _forum_session、CSRF)可能已整体失效,仅换 clearance 不等于恢复。
          // 后台强制重跑一次启动级 session bootstrap,把整包身份重建到与
          // 冷启动一致;_activeRefresh join 保证并发只跑一次。
